...
- Avoid decorative frames, ornate card borders, and model-drawn grid art that can mislead the crop overlay.
- Forbid text, labels, numbering, logos, watermarks, signatures, UI badges, and subjects crossing cell boundaries.
- In `single_target_variants`, keep one target identity across all cells while product-planned variation dimensions differ per cell; these cells are candidates, not emergent assets.
- When a sheet has reference images, record local reference metadata, upload through the configured provider, and pass returned URLs in the provider request.
...
- `row`
- `col`
- `kind`: `bound`, `emergent`, or `variant`
- `item_id`: required for bound, empty for emergent
- `title`
- `brief_snapshot`: required for bound
- `emergent_direction`: required for emergent
- `variant_direction`: required for variant; generated by the product planner rather than manually authored by the Agent
