...
| M5 | Demo打磨与展示 | 第5周后半 | 指标面板、导出、样例任务集、演示脚本/视频 | 可稳定完成演示，并能展示过程与指标 |
| M6 | 前后端分离与正式前端迁移 | vNext | 轻 API 层、正式 Web 前端、逐步弱化 Streamlit | 不重写核心流水线即可完成 UI 迁移 |
...
6. 在 M4 完成后，再进入 API 层与正式前端迁移，不在当前阶段继续重押 Streamlit 交互打磨。
